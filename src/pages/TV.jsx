  ...
  useEffect(() => {
    fetch(FIREBASE_URL)
      ...
      .then(data => {
        ...
        // Filter for only TV category
        const tvProducts = allProducts.filter(
          (product) => product?.category?.toLowerCase() === 'tv'
        );

        const shuffled = shuffleArray(tvProducts);
        setProducts({
    ...
    <div className="min-h-screen rounded-md bg-[#0B192C] flex flex-col">
      <div className="container mx-auto px-4 py-4">
        <h1 className="text-4xl text-center font-extrabold bg-gradient-to-r from-green-300 via-blue-500 to-purple-600 bg-clip-text text-transparent mb-4 font-mono">
          Explore the Smartest TVs for Your Home
        </h1>
        <p className="text-xl text-center text-gray-300 mb-8">
          Find the perfect smart TV with stunning visuals and immersive sound for every room.
        </p>
