  ...
  useEffect(() => {
    fetch(FIREBASE_URL)
      ...
      .then(data => {
        ...
        // Filter for only Washing Machine category
        const washingMachineProducts = allProducts.filter(
          (product) => product?.category?.toLowerCase() === 'washing machine'
        );

        const shuffled = shuffleArray(washingMachineProducts);
        setProducts({
    ...
    <div className="min-h-screen rounded-md bg-[#0B192C] flex flex-col">
      <div className="container mx-auto px-4 py-4">
        <h1 className="text-4xl text-center font-extrabold bg-gradient-to-r from-green-300 via-blue-500 to-purple-600 bg-clip-text text-transparent mb-4 font-mono">
          Smart Washing Machines for Modern Living
        </h1>
        <p className="text-xl text-center text-gray-300 mb-8">
          Explore efficient, silent, and stylish washing machines designed to make laundry effortless.
        </p>
