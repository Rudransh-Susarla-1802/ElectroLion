  ...
  useEffect(() => {
    fetch(FIREBASE_URL)
      ...
      .then(data => {
        ...
        // Filter for only Air Conditioner category
        const acProducts = allProducts.filter(
          (product) => product?.category?.toLowerCase() === 'air conditioner'
        );

        const shuffled = shuffleArray(acProducts);
        setProducts({
    ...
    <div className="min-h-screen rounded-md bg-[#0B192C] flex flex-col">
      <div className="container mx-auto px-4 py-4">
        <h1 className="text-4xl text-center font-extrabold bg-gradient-to-r from-green-300 via-blue-500 to-purple-600 bg-clip-text text-transparent mb-4 font-mono">
          Cool Comfort with Air Conditioners
        </h1>
        <p className="text-xl text-center text-gray-300 mb-8">
          Discover powerful and energy-efficient air conditioners perfect for every room and budget.
        </p>
